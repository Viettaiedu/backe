...
const register = (req, res) => {
  ...
  db.query(q, [req.body.email], (err, result) => {
    if (err)
      return res
        .status(404)
        .json({ message: "Error systax sql when query", error: err });
    if (result.length > 0)
      return res.status(404).json({ message: "Email is existing" });
    q =
    ...
    db.query(q, [values], (err, result) => {
      ...
      res
        .status(200)
        .json({ message: "INSERT USERS SUCCESSFULLY", user: result });
    });
  });
};
  ...
  db.query(q, [req.body.email], (err, result) => {
    if (err)
      return res
        ...
        .json({ message: "Error systax sql when query", error: err });
    if (result.length <= 0)
      return res.status(404).json({ message: "Email is not invalid" });
